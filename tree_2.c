...
int cmpres (TreeNodePtr p1, TreeNodePtr1 p2, int res)
{
    ...
    {	printf("DEBUG COMPARE more word node %d\t%d\n",p1->count,p2->count);    
        return 1;
    }	
    if (p1->count < p2->count) 
    {	
//	printf("DEBUG COMPARE less %d\t%d\n",p1->count,p2->count);
        return -1;
    } 
    if (p1->count = p2->count)
    {
  //      printf("DEBUG COMPARE equal %d\t%d\n",p1->count,p2->count);
        res=strcmp(p1->data, p2->data); // ili naoborot?))
        if (res > 0) return 1;
        if (res < 0) return -1;
    }      
}
...
void exchange(TreeNodePtr1 *TemPtr, TreeNodePtr treePtr)
{
    TreeNode1 *temp = NULL;
    if (*TemPtr == NULL)
    { 
	...
	printf("DEBUG BYPASS %d\t%s\n",temp->count,temp->data);

        *TemPtr = temp;
    } 
    else if (cmpres(treePtr,*TemPtr,r) < 0)
       {
//	printf("DEBUG EXCHANGE BYPASS left\n");
        exchange(&((*TemPtr)->leftPtr), treePtr);
       }
    else if (cmpres(treePtr,*TemPtr,r) > 0)
       {
//	printf("DEBUG EXCHANGE BYPASS right\n");
        exchange(&((*TemPtr)->rightPtr), treePtr);
       }	
    exch++;
    printf("DEBUG EXCHANGE COUNT %d\n", exch);
}
...
void bypass(TreeNodePtr1 TemPtr, TreeNodePtr treePtr )
{
    if (treePtr != NULL)
    {
        bypass(TemPtr,treePtr->leftPtr);
        exchange(&TemPtr,treePtr);
        bypass(TemPtr,treePtr->rightPtr);
	bps++;
        printf("DEBUG BYPASS COUNT %d\n", bps);
    }
}
...
{   
    if (treePtr != NULL )
    {
        ...
        outp++;
        printf("DEBUG OUTPUT COUNT %d\n", outp);
    }
}

void alphabetic2(TreeNodePtr1 TemPtr)
{   
     printf("DEBUG OUTPUT 2 COUNT %d\n", outp);	
    if (TemPtr != NULL )
    {
        ...
        outp++;
        printf("DEBUG OUTPUT 2 COUNT 2 %d\n", outp);
    }
}
